...
def XySplit(subjects,path,labels,image_folder,num_fixations):
    ...
    for subject in pos_subjects:
        ...
        for image in subject_images:
            ...
            trial_name = trials.append(image.strip('.png'))
...
class Image2Array:
  ...
  def __init__(self,subjects,path,labels,image_folder,num_fixations):
    ...
    X, y, names, trials = XySplit(self.subjects,self.path,self.labels,self.image_folder,self.num_fixations)
    self.X = np.array(X)
    self.y = np.array(y)
    self.names = names
    self.trials = trials
